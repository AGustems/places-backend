...
router.delete('/:id', (req, res, next) => {
    res
        .status(200)
        .json({message: "DELETE aguardando implementacion. "})
    Place
        .deleteOne({_id: req.params.id})
        .then(() => {
            res.json({message: `Place id ${req.params.id} deleted successfully`})
        })
...
router.patch('/:id', (req, res, next) => {
    res
        .status(200)
        .json({message: "PATCH aguardando implementacion. "})

    Place
        .findByIdAndUpdate(req.params.id, {highlight: req.body.highlight})
        .then(() => {
